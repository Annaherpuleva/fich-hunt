...
try {
  const ocean = await request(`${baseUrl}/api/v1/ocean/state`)
  assert.equal(ocean.response.status, 200)
  oceanStateContract.parse(ocean.body)

  const created = await request(`${baseUrl}/api/v1/fish/create`, {
    method: "POST",
    headers: authUser,
  ...
  const fishById = await request(`${baseUrl}/api/v1/fish/${createdFish.id}`)
  fishContract.parse(fishById.body)

  const myFish = await request(`${baseUrl}/api/v1/me/fish`, { headers: authUser })
  ...
  const fed = await request(`${baseUrl}/api/v1/fish/${createdFish.id}/feed`, {
    method: "POST",
    headers: authUser,
  ...
  const prey = await request(`${baseUrl}/api/v1/fish/create`, {
    method: "POST",
    headers: authUser,
  ...
  const placeMark = await request(`${baseUrl}/api/v1/fish/${fishAfterFeed.id}/place-mark`, {
    method: "POST",
    headers: authUser,
  ...
  const hunted = await request(`${baseUrl}/api/v1/fish/${fishAfterFeed.id}/hunt`, {
    method: "POST",
    headers: authUser,
  ...
  const preyExited = await request(`${baseUrl}/api/v1/fish/${preyFish.id}/exit`, {
    method: "POST",
    headers: authUser,
  ...
  const resurrect = await request(`${baseUrl}/api/v1/fish/${preyFish.id}/resurrect`, {
    method: "POST",
    headers: authUser,
  ...
  const transferred = await request(`${baseUrl}/api/v1/fish/${resurrected.id}/transfer`, {
    method: "POST",
    headers: authUser,
  ...
  const exited = await request(`${baseUrl}/api/v1/fish/${createdFish.id}/exit`, {
    method: "POST",
    headers: authUser,
  ...
  const depositIntentResponse = await request(`${baseUrl}/api/v1/payments/deposit-intent`, {
    method: "POST",
    headers: authUser,
  ...
  const inbound = await request(`${baseUrl}/api/v1/integrations/ton/inbound`, {
    method: "POST",
  ...
  await request(`${baseUrl}/api/v1/integrations/ton/sync`, { method: "POST" })

  const ledger = await request(`${baseUrl}/api/v1/me/ledger`, { headers: authUser })
  meLedgerContract.parse(ledger.body)

  const payments = await request(`${baseUrl}/api/v1/me/payments`, { headers: authUser })
  ...
  const approve = await request(`${baseUrl}/api/v1/admin/withdraw/${withdrawalId}/approve`, {
    method: "POST",
    headers: authAdmin,
  })
  ...
  const reject = await request(`${baseUrl}/api/v1/admin/withdraw/${withdrawalId2}/reject`, {
    method: "POST",
    headers: authAdmin,
  })
  adminRejectContract.parse(reject.body)

  const initOcean = await request(`${baseUrl}/api/v1/admin/ocean/init`, { method: "POST", headers: authAdmin })
  oceanStateContract.parse(initOcean.body)

  const rolloverOcean = await request(`${baseUrl}/api/v1/admin/ocean/rollover`, {
    method: "POST",
    headers: authAdmin,
  ...
  const manifestApi = await request(`${baseUrl}/api/v1/integrations/ton/manifest`)
